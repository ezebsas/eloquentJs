...
function storageFor(name) {
  ...
  // else if (name == "Chateau" || name == "Butcher Shop") storage.scalpel = "Woods"
...
class Node {
  ...
  send(to, type, message, callback) {
    ...
    if (Math.random() > 0.07) setTimeout(() => {
      try {
...
let chateau = network1.nodes["Chateau"]
...
function findInRemoteStorage(nest, name) {
  ...
  function next() {
    console.log('numero: ', 14 - sources.length);
    if (sources.length == 0) {
...
//
//   let sources = network(nest).filter(n => n != nest.name);
//   console.log(`first sources <${sources}> ${sources.length}`);
//   while (sources.length > 0) {
//     let source = sources[Math.floor(Math.random() *
//                                     sources.length)];
//     sources = sources.filter(n => n != source);
//     try {
//       console.log(`nest.name ${nest.name}, source ${source}, sources <<<${sources}>>>${sources.length}`);
//       let found = await routeRequest(nest, source, "storage",
//                                      name);
//       pasos ++;
//       console.log(`paso: ${pasos}, found: ${found}`);
//       if (found != null) return found;
//     } catch (_) {}
//   }
...
// let pr =storage(bigOak, "enemies")
//   pr.then(value => console.log("Got", value));
//   pr.then(value => console.log("Hay: ", value));
//
// pCaches = storage(bigOak, "food caches")
//   .then(function(dataCaches){
...
//   .then(arrays => console.log("vecinos disponibles: ",arrays))


// sendGossip(bigOak, "Kids with airgun in the park");

*/
// setTimeout(() => {routeRequest(bigOak, "Church Tower", "note", "Incoming jackdaws!")}, 5000)
// routeRequest(bigOak, "Church Tower", "note", "Incoming jackdaws!")
//   .then(v => console.log(v) )
//   .catch(e => console.log('error: ', e))

// request(bigOak,"Cow Pasture", "route", {target: 'Chateau',type: "note",content: "hola"})
//   .catch(e => console.log('Error y ahora?!!'))
// setTimeout(() =>request(bigOak,"Gilles' Garden", "route", {target: 'Church Tower',type: "note",content: "hola"}), 5000)
// setTimeout(() =>routeRequest(bigOak, "Church Tower", "note", "Incoming jackdaws!"), 5000)

// setTimeout(() =>findInStorage(bigOak, "events on 2017-12-21").then(console.log).catch(), 5000)

// storage(chateau, "food caches")
...
// storage(chateau, 'events on 2017-12-21').then(v => console.log(v));

// setTimeout(() => chicks(bigOak, 2017).then(v => console.log(v)).catch(e => console.log(e)), 5000)
...
async function locateScalpel(nest) {
  await timeout(3000)
  let array = network(nest)
  for (let current of array) {
    let place = await anyStorage(nest, current, 'scalpel');
    if(current == place) return current;
  }
}
...
function locateScalpel2(nest) {
  ...
  function next() {
    ...
    sources = sources.filter(v => v != source)
    return anyStorage(nest, source, "scalpel")
      .then(value => value == source ? source : next(),
        next);
...
setTimeout(() => locateScalpel2(bigOak).then(console.log), 1000);
